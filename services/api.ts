...
export const validateFace = async (params: ValidateFaceParams): Promise<ValidateFaceResponse> => {
  try {
    const response = await fetch(API_BASE_URL, {
      method: 'POST',
      headers: {
...
export const validateQRCode = async ({ email, f1, address, qr }: ValidateQRParams): Promise<ValidateQRResponse> => {
  try {
    const response = await fetch(API_BASE_URL, {
      method: 'POST',
      headers: {
