...
export const loadUser = createAsyncThunk("auth/loadUser", async () => {
	try {
		...
		if (availableUsersJson) {
			...
			console.log(
				"Usuarios disponibles cargados desde AsyncStorage:",
			...
			await AsyncStorage.setItem(
				AVAILABLE_USERS_KEY,
				JSON.stringify(initialState.availableUsers),
			);
			console.log(
				"Usuarios iniciales guardados en AsyncStorage:",
				initialState.availableUsers.length,
			);
		}
		...
		return {
			user: null,
			availableUsers: initialState.availableUsers,
			lastSyncDate: null,
		};
	}
});
...
export const { login, logout, setLoading, setAvailableUsers } =
	authSlice.actions;
...
export const selectAvailableUsers = (state: { auth: AuthState }) =>
	state.auth.availableUsers;
export const selectAuthLastSyncDate = (state: { auth: AuthState }) =>
	state.auth.lastSyncDate;
